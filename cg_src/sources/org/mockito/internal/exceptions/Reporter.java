...
/* loaded from: gencallgraphv3.jar:mockito-core-3.6.0.jar:org/mockito/internal/exceptions/Reporter.class */
public class Reporter {
    ...
    public static MockitoException checkedExceptionInvalid(Throwable t) {
        return new MockitoException(StringUtil.join("Checked exception is invalid for this method!", "Invalid: " + t));
    }
    ...
    public static MockitoException notAMockPassedToVerify(Class<?> type) {
        return new NotAMockException(StringUtil.join("Argument passed to verify() is of type " + type.getSimpleName() + " and is not a mock!", "Make sure you place the parenthesis correctly!", "See the examples of correct verifications:", "    verify(mock).someMethod();", "    verify(mock, times(10)).someMethod();", "    verify(mock, atLeastOnce()).someMethod();"));
    }
    ...
    public static MockitoException stubPassedToVerify(Object mock) {
        return new CannotVerifyStubOnlyMock(StringUtil.join("Argument \"" + MockUtil.getMockName(mock) + "\" passed to verify is a stubOnly() mock which cannot be verified.", "If you intend to verify invocations on this mock, don't use stubOnly() in its MockSettings."));
    }

    public static MockitoException reportNoSubMatchersFound(String additionalMatcherName) {
        return new InvalidUseOfMatchersException(StringUtil.join("No matchers found for additional matcher " + additionalMatcherName, new LocationImpl(), ""));
    }
    ...
    private static Object locationsOf(Collection<LocalizedMatcher> matchers) {
        ...
        return StringUtil.join(description.toArray());
    }
    ...
    public static MockitoAssertionError wantedButNotInvoked(DescribedInvocation wanted, List<? extends DescribedInvocation> invocations) {
        ...
        if (invocations.isEmpty()) {
            ...
            StringBuilder sb = new StringBuilder("\nHowever, there " + Pluralizer.were_exactly_x_interactions(invocations.size()) + " with this mock:\n");
    ...
    private static String createWantedButNotInvokedMessage(DescribedInvocation wanted) {
        return StringUtil.join("Wanted but not invoked:", wanted.toString(), new LocationImpl(), "");
    }

    public static MockitoAssertionError wantedButNotInvokedInOrder(DescribedInvocation wanted, DescribedInvocation previous) {
        return new VerificationInOrderFailure(StringUtil.join("Verification in order failure", "Wanted but not invoked:", wanted.toString(), new LocationImpl(), "Wanted anywhere AFTER following interaction:", previous.toString(), previous.getLocation(), ""));
    }
    ...
    private static String createTooManyInvocationsMessage(int wantedCount, int actualCount, DescribedInvocation wanted, List<Location> invocations) {
        return StringUtil.join(wanted.toString(), "Wanted " + Pluralizer.pluralize(wantedCount) + ":", new LocationImpl(), "But was " + Pluralizer.pluralize(actualCount) + ":", createAllLocationsMessage(invocations), "");
    }

    public static MockitoAssertionError neverWantedButInvoked(DescribedInvocation wanted, List<Location> invocations) {
        return new NeverWantedButInvoked(StringUtil.join(wanted.toString(), "Never wanted here:", new LocationImpl(), "But invoked here:", createAllLocationsMessage(invocations)));
    }
    ...
    public static MockitoAssertionError tooManyActualInvocationsInOrder(int wantedCount, int actualCount, DescribedInvocation wanted, List<Location> invocations) {
        ...
        return new VerificationInOrderFailure(StringUtil.join("Verification in order failure:" + message));
    }
    ...
    private static String createTooFewInvocationsMessage(Discrepancy discrepancy, DescribedInvocation wanted, List<Location> locations) {
        ...
        objArr[0] = wanted.toString();
        objArr[1] = "Wanted " + discrepancy.getPluralizedWantedCount() + (discrepancy.getWantedCount() == 0 ? "." : ":");
        objArr[2] = new LocationImpl();
        objArr[3] = "But was " + discrepancy.getPluralizedActualCount() + (discrepancy.getActualCount() == 0 ? "." : ":");
        objArr[4] = createAllLocationsMessage(locations);
    ...
    public static MockitoAssertionError noMoreInteractionsWanted(Invocation undesired, List<VerificationAwareInvocation> invocations) {
        ...
        return new NoInteractionsWanted(StringUtil.join("No interactions wanted here:", new LocationImpl(), "But found this interaction on mock '" + MockUtil.getMockName(undesired.getMock()) + "':", undesired.getLocation(), scenario));
    }

    public static MockitoAssertionError noMoreInteractionsWantedInOrder(Invocation undesired) {
        return new VerificationInOrderFailure(StringUtil.join("No interactions wanted here:", new LocationImpl(), "But found this interaction on mock '" + MockUtil.getMockName(undesired.getMock()) + "':", undesired.getLocation()));
    }
    ...
    public static MockitoAssertionError noInteractionsWanted(Object mock, List<VerificationAwareInvocation> invocations) {
        ...
        return new NoInteractionsWanted(StringUtil.join("No interactions wanted here:", new LocationImpl(), "But found these interactions on mock '" + MockUtil.getMockName(mock) + "':", StringUtil.join("", locations), scenario));
    }

    public static MockitoException cannotMockClass(Class<?> clazz, String reason) {
        return new MockitoException(StringUtil.join("Cannot mock/spy " + clazz.toString(), "Mockito cannot mock/spy because :", " - " + reason));
    }

    public static MockitoException cannotStubVoidMethodWithAReturnValue(String methodName) {
        return new CannotStubVoidMethodWithReturnValue(StringUtil.join("'" + methodName + "' is a *void method* and it *cannot* be stubbed with a *return value*!", "Voids are usually stubbed with Throwables:", "    doThrow(exception).when(mock).someVoidMethod();", "If you need to set the void method to do nothing you can use:", "    doNothing().when(mock).someVoidMethod();", "For more information, check out the javadocs for Mockito.doNothing().", "***", "If you're unsure why you're getting above error read on.", "Due to the nature of the syntax above problem might occur because:", "1. The method you are trying to stub is *overloaded*. Make sure you are calling the right overloaded version.", "2. Somewhere in your test you are stubbing *final methods*. Sorry, Mockito does not verify/stub final methods.", "3. A spy is stubbed using when(spy.foo()).then() syntax. It is safer to stub spies - ", "   - with doReturn|Throw() family of methods. More in javadocs for Mockito.spy() method.", "4. Mocking methods declared on non-public parent classes is not supported.", ""));
    }
    ...
    public static MockitoException wrongTypeReturnedByDefaultAnswer(Object mock, String expectedType, String actualType, String methodName) {
        return new WrongTypeOfReturnValue(StringUtil.join("Default answer returned a result with the wrong type:", actualType + " cannot be returned by " + methodName + "()", methodName + "() should return " + expectedType, "", "The default answer of " + MockUtil.getMockName(mock) + " that was configured on the mock is probably incorrectly implemented.", ""));
    }

    public static MoreThanAllowedActualInvocations wantedAtMostX(int maxNumberOfInvocations, int foundSize) {
        return new MoreThanAllowedActualInvocations(StringUtil.join("Wanted at most " + Pluralizer.pluralize(maxNumberOfInvocations) + " but was " + foundSize));
    }
    ...
    public static MockitoException smartNullPointerException(String invocation, Location location) {
        return new SmartNullPointerException(StringUtil.join("You have a NullPointerException here:", new LocationImpl(), "because this method call was *not* stubbed correctly:", location, invocation, ""));
    }
    ...
    public static MockitoException extraInterfacesAcceptsOnlyInterfaces(Class<?> wrongType) {
        return new MockitoException(StringUtil.join("extraInterfaces() accepts only interfaces.", "You passed following type: " + wrongType.getSimpleName() + " which is not an interface."));
    }

    public static MockitoException extraInterfacesCannotContainMockedType(Class<?> wrongType) {
        return new MockitoException(StringUtil.join("extraInterfaces() does not accept the same type as the mocked type.", "You mocked following type: " + wrongType.getSimpleName(), "and you passed the same very interface to the extraInterfaces()"));
    }
    ...
    public static MockitoException cannotInitializeForSpyAnnotation(String fieldName, Exception details) {
        return new MockitoException(StringUtil.join("Cannot instantiate a @Spy for '" + fieldName + "' field.", "You haven't provided the instance for spying at field declaration so I tried to construct the instance.", "However, I failed because: " + details.getMessage(), "Examples of correct usage of @Spy:", "   @Spy List mock = new LinkedList();", "   @Spy Foo foo; //only if Foo has parameterless constructor", "   //also, don't forget about MockitoAnnotations.openMocks();", ""), details);
    }

    public static MockitoException cannotInitializeForInjectMocksAnnotation(String fieldName, String causeMessage) {
        return new MockitoException(StringUtil.join("Cannot instantiate @InjectMocks field named '" + fieldName + "'! Cause: " + causeMessage, "You haven't provided the instance at field declaration so I tried to construct the instance.", "Examples of correct usage of @InjectMocks:", "   @InjectMocks Service service = new Service();", "   @InjectMocks Service service;", "   //and... don't forget about some @Mocks for injection :)", ""));
    }
    ...
    public static MockitoException invocationListenerThrewException(InvocationListener listener, Throwable listenerThrowable) {
        return new MockitoException(StringUtil.join("The invocation listener with type " + listener.getClass().getName(), "threw an exception : " + listenerThrowable.getClass().getName() + listenerThrowable.getMessage()), listenerThrowable);
    }

    public static MockitoException cannotInjectDependency(Field field, Object matchingMock, Exception details) {
        return new MockitoException(StringUtil.join("Mockito couldn't inject mock dependency '" + MockUtil.getMockName(matchingMock) + "' on field ", "'" + field + "'", "whose type '" + field.getDeclaringClass().getCanonicalName() + "' was annotated by @InjectMocks in your test.", "Also I failed because: " + exceptionCauseMessageIfAvailable(details), ""), details);
    }
    ...
    public static MockitoException mockedTypeIsInconsistentWithDelegatedInstanceType(Class<?> mockedType, Object delegatedInstance) {
        return new MockitoException(StringUtil.join("Mocked type must be the same as the type of your delegated instance.", "Mocked type must be: " + delegatedInstance.getClass().getSimpleName() + ", but is: " + mockedType.getSimpleName(), "  //correct delegate:", "  spy = mock( ->List.class<- , withSettings().delegatedInstance( ->new ArrayList()<- );", "  //incorrect - types don't match:", "  spy = mock( ->List.class<- , withSettings().delegatedInstance( ->new HashSet()<- );"));
    }
    ...
    public static MockitoException invalidArgumentPositionRangeAtInvocationTime(InvocationOnMock invocation, boolean willReturnLastParameter, int argumentIndex) {
        ...
        objArr[1] = " -> " + MockUtil.getMockName(invocation.getMock()) + "." + invocation.getMethod().getName() + "()";
        objArr[2] = "";
        StringBuilder sb = new StringBuilder();
        if (willReturnLastParameter) {
            str = "Last parameter wanted";
        } else {
            str = "Wanted parameter at position " + argumentIndex;
        }
        objArr[3] = sb.append(str).append(" but ").append((Object) possibleArgumentTypesOf(invocation)).toString();
    ...
    public static MockitoException delegatedMethodDoesNotExistOnDelegate(Method mockMethod, Object mock, Object delegate) {
        return new MockitoException(StringUtil.join("Methods called on mock must exist in delegated instance.", "When calling: " + mockMethod + " on mock: " + MockUtil.getMockName(mock), "no such method was found.", "Check that the instance passed to delegatesTo() is of the correct type or contains compatible methods", "(delegate instance had type: " + delegate.getClass().getSimpleName() + ")"));
    }
    ...
    public static MockitoException cannotCreateTimerWithNegativeDurationTime(long durationMillis) {
        return new FriendlyReminderException(StringUtil.join("", "Don't panic! I'm just a friendly reminder!", "It is impossible for time to go backward, therefore...", "You cannot put negative value of duration: (" + durationMillis + ")", "as argument of timer methods (after(), timeout())", ""));
    }
    ...
    public static UnnecessaryStubbingException formatUnncessaryStubbingException(Class<?> testClass, Collection<Invocation> unnecessaryStubbings) {
        ...
        for (Invocation u : unnecessaryStubbings) {
            int i = count;
            count++;
            stubbings.append("\n  ").append(i).append(". ").append(u.getLocation());
        }
        if (testClass != null) {
            str = "Unnecessary stubbings detected in test class: " + testClass.getSimpleName();
        } else {
            str = "Unnecessary stubbings detected.";
        }
        String heading = str;
        return new UnnecessaryStubbingException(StringUtil.join(heading, "Clean & maintainable test code requires zero unnecessary code.", "Following stubbings are unnecessary (click to navigate to relevant line of code):" + ((Object) stubbings), "Please remove unnecessary stubbings or use 'lenient' strictness. More info: javadoc for UnnecessaryStubbingException class."));
    }
