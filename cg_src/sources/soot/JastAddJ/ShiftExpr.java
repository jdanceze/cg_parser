...
/* loaded from: gencallgraphv3.jar:soot/JastAddJ/ShiftExpr.class */
public abstract class ShiftExpr extends Binary implements Cloneable {
    ...
    public void typeCheck() {
        if (!getLeftOperand().type().isIntegralType()) {
            error(String.valueOf(getLeftOperand().type().typeName()) + " is not integral");
        }
        if (!getRightOperand().type().isIntegralType()) {
            error(String.valueOf(getRightOperand().type().typeName()) + " is not integral");
        }
    }
