...
/* loaded from: gencallgraphv3.jar:soot/dexpler/instructions/ConditionalJumpInstruction.class */
public abstract class ConditionalJumpInstruction extends JumpInstruction implements DeferableInstruction {
    ...
    public void deferredJimplify(DexBody body) {
        IfStmt s = ifStatement(body);
        body.getBody().getUnits().swapWith(this.markerUnit, (Unit) s);
        setUnit(s);
    }

    /* JADX INFO: Access modifiers changed from: protected */
    public ConditionExpr getComparisonExpr(DexBody body, int reg) {
        Local one = body.getRegisterLocal(reg);
        return getComparisonExpr(one, IntConstant.v(0));
    }

    /* JADX INFO: Access modifiers changed from: protected */
    public ConditionExpr getComparisonExpr(Immediate one, Immediate other) {
